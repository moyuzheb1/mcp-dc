...
export const useChatStore = defineStore("chat", () => {
  ...
  >([]);
  const messagesMap = ref<Map<number, Array<AssistantMessage | UserMessage>>>(
    new Map(),
  );
  ...
  // 使用Map来存储会话工作状态
  const threadsWorkingStatusMap = ref<Map<number, Map<string, WorkingStatus>>>(
    new Map(),
  );
  ...
  const generatingMessagesCacheMap = ref<
    Map<
      number,
      Map<string, { message: AssistantMessage | UserMessage; threadId: string }>
    >
  >(new Map());
  ...
  const getActiveThreadId = () =>
    activeThreadIdMap.value.get(getTabId()) ?? null;
  ...
  const createNewEmptyThread = async () => {
    try {
      ...
      console.error(
        "Failed to clear active thread and load first page:",
        error,
      );
      throw error;
    }
  ...
  const createThread = async (
    ...
    try {
      const threadId = await threadP.createConversation(
        title,
        settings,
        getTabId(),
      );
  ...
  const enrichMessageWithExtra = async (message: Message): Promise<Message> => {
    if (
      ...
      const attachments = await threadP.getMessageExtraInfo(
        message.id,
        "search_result",
      );
      // 更新消息中的 extra 信息
      ...
      if (assistantMessage.variants && assistantMessage.variants.length > 0) {
        assistantMessage.variants = await Promise.all(
          assistantMessage.variants.map((variant) =>
            enrichMessageWithExtra(variant),
          ),
        );
      }
    }
      ...
      for (const [, cached] of getGeneratingMessagesCache()) {
        if (cached.threadId === getActiveThreadId()) {
          ...
          if (message.is_variant && message.parentId) {
            ...
            const parentMsg = mergedMessages.find(
            ...
            if (parentMsg) {
              ...
              const existingVariantIndex = parentMsg.variants.findIndex(
                (v) => v.id === message.id,
              );
              if (existingVariantIndex !== -1) {
                parentMsg.variants[existingVariantIndex] =
                  await enrichMessageWithExtra(message);
              } else {
  ...
  const retryMessage = async (messageId: string) => {
    if (!getActiveThreadId()) return;
    try {
      const aiResponseMessage = await threadP.retryMessage(
        messageId,
        chatConfig.value.modelId,
      );
      // 将消息添加到缓存
      getGeneratingMessagesCache().set(aiResponseMessage.id, {
        message: aiResponseMessage,
    ...
    if (cached) {
      ...
      if (curMsg.content) {
        ...
        const finalizeLastBlock = () => {
          const lastBlock =
            curMsg.content.length > 0
              ? curMsg.content[curMsg.content.length - 1]
              : undefined;
          if (lastBlock) {
  ...
  const handleStreamEnd = async (msg: { eventId: string }) => {
    ...
    if (cached) {
      ...
      if (updatedMessage && updatedMessage.role === "assistant") {
        // 使用异步执行，不阻塞主流程
        void saveAssistantMessageToLocal(
          msg.eventId,
          cached.threadId,
          "markdown",
        ).catch((err) => {
          console.error(`Failed to auto-save assistant response:`, err);
        });
      }
      ...
      if (enrichedMessage.is_variant && enrichedMessage.parentId) {
        ...
        if (mainMessage) {
          ...
          if (getActiveThreadId() === getActiveThreadId()) {
            const mainMsgIndex = getMessages().findIndex(
              (m) => m.id === mainMessage.id,
            );
            if (mainMsgIndex !== -1) {
              getMessages()[mainMsgIndex] = enrichedMainMessage as
                | AssistantMessage
                | UserMessage;
            }
          }
        ...
        if (getActiveThreadId() === getActiveThreadId()) {
          const msgIndex = getMessages().findIndex((m) => m.id === msg.eventId);
          if (msgIndex !== -1) {
            getMessages()[msgIndex] = enrichedMessage as
              | AssistantMessage
              | UserMessage;
          }
        }
  ...
  const handleStreamError = async (msg: { eventId: string }) => {
    ...
    if (cached) {
      try {
        ...
        if (enrichedMessage.is_variant && enrichedMessage.parentId) {
          // 处理变体消息的错误状态
          const parentMsgIndex = getMessages().findIndex(
          ...
          if (parentMsgIndex !== -1) {
            ...
            const variantIndex = parentMsg.variants.findIndex(
              (v) => v.id === enrichedMessage.id,
            );
            if (variantIndex !== -1) {
          ...
          // 非变体消息的原有错误处理逻辑
          const messageIndex = getMessages().findIndex(
            (m) => m.id === msg.eventId,
          );
          if (messageIndex !== -1) {
            getMessages()[messageIndex] = enrichedMessage as
              | AssistantMessage
              | UserMessage;
          }
        }
        ...
        if (!isFocused) {
          ...
          if (
            enrichedMessage &&
            (enrichedMessage as AssistantMessage).content
          ) {
            const assistantMsg = enrichedMessage as AssistantMessage;
            // 查找错误信息块
            for (const block of assistantMsg.content) {
  ...
  const saveChatConfig = async () => {
    if (!getActiveThreadId()) return;
    try {
      await threadP.updateConversationSettings(
        getActiveThreadId()!,
        chatConfig.value,
      );
    } catch (error) {
  ...
  const updateChatConfig = async (
    newConfig: Partial<CONVERSATION_SETTINGS>,
  ) => {
  ...
  const deleteMessage = async (messageId: string) => {
    ...
    try {
      ...
      if (
        mainMsgIndex !== -1 &&
        (messages[mainMsgIndex] as AssistantMessage).is_variant === 0
      ) {
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          if (
            msg.role === "assistant" &&
            msg.variants?.some((v) => v.id === messageId)
          ) {
            parentMessage = msg;
            parentIndex = i;
            break;
        ...
        if (parentMessage && parentIndex !== -1) {
          const remainingVariants =
            parentMessage.variants?.filter((v) => v.id !== messageId) || [];
          ...
          const newSelectedVariantId =
            remainingVariants.length > 0
              ? remainingVariants[remainingVariants.length - 1].id
              : null;
  ...
  window.electron.ipcRenderer.on(
    ...
    async (_, data: { userInput: string }) => {
      ...
      if (!getActiveThreadId()) {
        console.error(
          "Received send-initial-message command but no active thread is set.",
        );
        return;
      }
  ...
  const updateSelectedVariant = async (
    ...
    if (chatConfig.value) {
      chatConfig.value.selectedVariantsMap = Object.fromEntries(
  ...
  window.electron.ipcRenderer.on(DEEPLINK_EVENTS.START, async (_, data) => {
    console.log(
      `[Renderer] Tab ${getTabId()} received DEEPLINK_EVENTS.START:`,
      data,
    );
    // 确保路由正确
  ...
  const updateThreadWorkingStatus = (
    threadId: string,
    ...
    window.electron.ipcRenderer.on(
      ...
      (_, updatedGroupedList: { dt: string; dtThreads: CONVERSATION[] }[]) => {
        ...
        if (currentActiveId) {
          ...
          const activeThreadExists = flatList.some(
            (thread) => thread.id === currentActiveId,
          );
  ...
  onMounted(() => {
    console.log(
      `[Chat Store] Tab ${getTabId()} is mounted. Setting up event listeners.`,
    );
   ...
   * 主线程导出
   */
  const exportWithMainThread = async (
    threadId: string,
   ...
   */
  const saveAssistantMessageToLocal = async (
    messageId: string,
    threadId: string,
    format: "markdown" | "html" | "txt" = "markdown",
  ): Promise<void> => {
    try {
      ...
      if (!messageId || !threadId) {
        console.error("Failed to save message: Missing required parameters");
        return;
      }
      ...
      if (!conversation) {
        console.error(
          `Failed to save message: Conversation ${threadId} not found`,
        );
        return;
      }

      // 获取并验证助手消息
      const message = await threadP.getMessage(messageId);
      if (!message || message.role !== "assistant") {
        console.error(
          `Failed to save message: Message ${messageId} is not a valid assistant message`,
        );
        return;
      }
      ...
      switch (format) {
        case "markdown":
          filename = `assistant-response-${timestamp}.md`;
          content = `# 助手回复 (${new Date().toLocaleString()})\n\n`;
          break;
        case "txt":
          filename = `assistant-response-${timestamp}.txt`;
          content = `助手回复 (${new Date().toLocaleString()})\n`;
          content += "=".repeat(50) + "\n\n";
          break;
        case "html":
          filename = `assistant-response-${timestamp}.html`;
          content = `<!DOCTYPE html>\n<html>\n<head>\n<meta charset="UTF-8">\n<title>助手回复</title>\n</head>\n<body>\n<h1>助手回复 (${new Date().toLocaleString()})</h1>\n<div>`;
          break;
      }
        ...
        if (typeof message.content === "string") {
          ...
          for (const block of message.content) {
            if (block.type === "content" || block.type === "text") {
              ...
              content += format === "html" ? text.replace(/\n/g, "<br>") : text;
            }
          }
        }
      }
      ...
      // 检查window.api是否可用
      if (!window.api || typeof window.api.writeLocalFile !== "function") {
        console.error(
          "Failed to save message: window.api.writeLocalFile is not available",
        );
        return;
      }

      // 使用window.api.writeLocalFile保存到应用根目录
      console.log(`Attempting to save assistant response to: ${filename}`);
      const success = await window.api.writeLocalFile(filename, content);

      if (success) {
        console.log(
          `Successfully saved assistant response to application root directory: ${filename}`,
        );
      } else {
        console.error(
          `Failed to save assistant response to ${filename}: API returned failure`,
        );
      }
    } catch (error) {
      console.error(
        "Error auto-saving assistant response:",
        error instanceof Error ? error.message : String(error),
      );
      // 错误已记录，不需要重新抛出以避免影响主流程
    }
  };
