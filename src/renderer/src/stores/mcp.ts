...
export const useMcpStore = defineStore("mcp", () => {
  ...
  const toolResults = ref<
    Record<string, string | { type: string; text: string }[]>
  >({});
  ...
  const serverList = computed(() => {
    const servers = Object.entries(config.value.mcpServers).map(
      ([name, serverConfig]) => ({
        name,
        ...serverConfig,
  ...
  // 计算默认服务器数量
  const defaultServersCount = computed(
    () => config.value.defaultServers.length,
  );

  // 如果用户没有配置任何默认服务器，则视为默认启用所有已配置的 MCP servers
  // effectiveDefaultServers 用于前端显示或逻辑决策（不会修改持久配置）
  const effectiveDefaultServers = computed(() => {
    const configured = config.value.defaultServers;
    if (Array.isArray(configured) && configured.length > 0) return configured;
    return Object.keys(config.value.mcpServers || {});
  });

  // 用于 UI 提示：当未配置默认服务器但存在已配置的 server 时，显示“默认启用所有服务”的提示
  const showDefaultAllEnabledHint = computed(() => {
    return (
      Array.isArray(config.value.defaultServers) &&
      config.value.defaultServers.length === 0 &&
      Object.keys(config.value.mcpServers || {}).length > 0
    );
  });
  ...
  const updateServerStatus = async (
    ...
    try {
      serverStatuses.value[serverName] =
        await mcpPresenter.isServerRunning(serverName);
      ...
      console.error(
        t("mcp.errors.getServerStatusFailed", { serverName }),
        error,
      );
    ...
    try {
      ...
      if (config.value.defaultServers.includes(serverName)) {
        ...
        if (hasMaxDefaultServers.value) {
          ...
          return {
            success: false,
            message: t("mcp.errors.maxDefaultServersReached"),
          };
        }
    ...
    try {
      ...
      try {
        const configPrompts: Prompt[] =
          await configPresenter.getCustomPrompts();
  ...
  const callTool = async (toolName: string) => {
    try {
      ...
      toolResults.value[toolName] = t("mcp.errors.toolCallError", {
        error: String(error),
      });
      throw error;
    } finally {
  ...
  const init = async () => {
    ...
    if (!chatStore.getActiveThreadId()) {
      chatStore.chatConfig.enabledMcpTools = tools.value.map(
  ...
  // 设置自定义NPM Registry
  const setCustomNpmRegistry = async (
  ...
  return {
    ...
    // 默认服务器相关（前端提示与决策）
    effectiveDefaultServers,
    showDefaultAllEnabledHint,
